...
class TacticsResNet(nn.Module):
    ...
    """
    def __init__(self, num_classes, in_channels=26, num_filters=128, num_blocks=8, drop=0.4):
        super().__init__()
...
class ImprovedTacticCNN(nn.Module):
    def __init__(self, num_classes, drop=0.5):
        super().__init__()
        # initial conv - now accepts 26 channels instead of 22
        self.init = nn.Sequential(
            nn.Conv2d(26, 64, 3, padding=1),
            nn.BatchNorm2d(64),
            nn.ReLU()
        )
